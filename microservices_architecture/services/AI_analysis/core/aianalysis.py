...
#         azure_endpoint="https://aif-creditai-qa-001.cognitiveservices.azure.com/",
#         api_key="Au4vNbOXpEX2DPvxp7eH7CX5utrj5PE043biXOss78GyizT8fNMHJQQJ99BJACHYHv6XJ3w3AAAAACOGvY4U",
#         embedding_name="text-embedding-3-large",
#         embedding_deployment="text-embedding-3-large-creditai-qa",
#         embedding_api_version="2024-02-01",
#         model_name="gpt-5",
#         model_api_version="2024-12-01-preview",
#         openai_deployment="gpt-5-creditai-qa",
...
# # # #     # "2) Ensure all numeric details are correct, consistent, and neatly represented.\n"
# # # #     # "3) Highlight any factual or stylistic issues clearly."
# # # #     "Your task is to critically evaluate the generator agent’s response using the original prompt and context provided to it.\n\n"
# # # #     "1) Source Alignment Check:\n"
# # # #     "   - Verify that every statement in the generated answer is directly supported by the original prompt/context.\n"
# # # #     "   - Flag any hallucinations, assumptions, or information not present or not inferable from the prompt.\n\n"
# # # #     "2) Accuracy & Consistency Check:\n"
# # # #     "   - Validate all facts, logic, and numeric details.\n"
# # # #     "   - Confirm that no details were altered, invented, or lost.\n"
# # # #     "   - Highlight any inconsistencies between the prompt and the generated response.\n\n"
# # # #     "3) Clarity, Structure, and Completeness:\n"
# # # #     "   - Point out unclear, incomplete, or poorly structured areas.\n"
# # # #     "   - Identify if the response misses anything essential asked in the prompt.\n\n"
# # # #     "4) Output Format:\n"
# # # #     "   - Provide your findings clearly, referencing specific lines or sections if needed.\n"
...
# # # def make_pipeline_id(base: str, uid: str = None) -> str:
# # #     """
# # #     Creates a unique pipeline ID.
# # #     Example output: pipeline_customer_report_1c2f9b31e9144ad6a9cb1f5c0ea8f944
# # #     """
# # #     if not base:
# # #         base = "pipeline"

# # #     # Clean the base to avoid invalid characters
# # #     base = re.sub(r'[^a-zA-Z0-9]+', '_', base.lower())

# # #     unique_id = uid or uuid.uuid4().hex

# # #     return f"pipeline_{base}_{unique_id}"
...
# # #     """
# # #     Extracts a meaningful symbol/identifier from document content.
# # #     Used when filename is generic (e.g., 'document.pdf').
# # #     """
# # #     if not text:
# # #         return "unknown"

# # #     # Try extracting a title-like pattern (first line with words)
# # #     first_line = text.strip().split("\n")[0]

# # #     # Keep alphanumeric words only
# # #     cleaned = re.sub(r'[^a-zA-Z0-9 ]+', '', first_line).strip()

# # #     if cleaned:
# # #         cleaned = "_".join(cleaned.lower().split()[:7])
# # #         return cleaned

# # #     # Fallback: first 5 words from the entire text
# # #     words = re.findall(r"[a-zA-Z0-9]+", text.lower())
# # #     if words:
# # #         return "_".join(words[:7])

# # #     return "unknown"


# # # def extract_symbol_from_url_or_filename(path: str) -> str:
# # #     """
# # #     Extracts a symbol (e.g., 'AAPL', 'INFY', 'NIFTY50') from a URL or filename.

# # #     Works for:
# # #     - https://example.com/data/AAPL.json
# # #     - https://api.test.com/?symbol=TSLA
# # #     - C:/files/INFY_report.pdf
# # #     - ./data/NIFTY-50.txt
# # #     """
# # #     if not path:
# # #         return ""
...
# # #     # --- CASE 1: Try query param format: ?symbol=XXXX
# # #     match = re.search(r"(?:symbol|ticker|code)=([A-Za-z0-9\-_]+)", path, re.IGNORECASE)
# # #     if match:
# # #         return match.group(1).upper()

# # #     # --- CASE 2: Remove URL structure and keep last part
# # #     parsed = urlparse(path)
# # #     filename = parsed.path.split("/")[-1]  # last part of URL/filename

# # #     # Remove file extension if present
# # #     filename_no_ext = re.sub(r"\.[A-Za-z0-9]+$", "", filename)

# # #     # Now extract a symbol-like pattern
# # #     symbol = extract_symbol_from_text(filename_no_ext)
...
# # #         start_ts = time.time()
# # #         run_cid = str(uuid.uuid4())
# # #         ai_trace("fetch_and_parse_10q_start", properties={"correlation_id": run_cid, "url": url, "pipelineId": self.pipeline_id})
# # #         def blocking_fetch() -> str:
# # #             headers = {
# # #                 "User-Agent": "Sachin Bhusnurmath (sachin@example.com)",
# # #                 "Accept-Encoding": "gzip, deflate",
...
# # #             text = await asyncio.get_event_loop().run_in_executor(self.executor, blocking_fetch)
# # #             elapsed = time.time() - start_ts
# # #             ai_trace("fetch_and_parse_10q_success", properties={"correlation_id": run_cid, "elapsed_s": elapsed, "url": url, "pipelineId": self.pipeline_id})
# # #             return text
# # #         except Exception as e:
# # #             elapsed = time.time() - start_ts
# # #             ai_exception(e, properties={"correlation_id": run_cid, "elapsed_s": elapsed, "url": url, "pipelineId": self.pipeline_id})
# # #             raise

# # #     async def _build_vector_index(self, text: str):
# # #         start_ts = time.time()
# # #         run_cid = str(uuid.uuid4())
# # #         ai_trace("build_vector_index_start", properties={"correlation_id": run_cid, "text_len": len(text), "pipelineId": self.pipeline_id})
# # #         def blocking_build():
# # #             doc = Document(text=text)
# # #             node_parser = SimpleNodeParser.from_defaults(chunk_size=1024, chunk_overlap=256)
# # #             nodes = node_parser.get_nodes_from_documents([doc])
...
# # #             self.vector_index = index
# # #             elapsed = time.time() - start_ts
# # #             ai_trace("build_vector_index_success", properties={"correlation_id": run_cid, "elapsed_s": elapsed, "node_count": len(index._store._nodes) if hasattr(index, "_store") else None, "pipelineId": self.pipeline_id})
# # #             return index
# # #         except Exception as e:
# # #             elapsed = time.time() - start_ts
# # #             ai_exception(e, properties={"correlation_id": run_cid, "elapsed_s": elapsed, "pipelineId": self.pipeline_id})
# # #             raise

# # #     async def query_vector_index(self, query: str, top_k: int = 3) -> str:
# # #         start_ts = time.time()
# # #         run_cid = str(uuid.uuid4())
# # #         ai_trace("query_vector_index_start", properties={"correlation_id": run_cid, "query": query, "top_k": top_k, "pipelineId": self.pipeline_id})
# # #         def blocking_query():
# # #             query_engine = self.vector_index.as_query_engine(
# # #                 llm=self.llm, similarity_top_k=top_k, response_mode="compact"
# # #             )
...
# # #             result = await asyncio.get_event_loop().run_in_executor(self.executor, blocking_query)
# # #             elapsed = time.time() - start_ts
# # #             ai_trace("query_vector_index_success", properties={"correlation_id": run_cid, "elapsed_s": elapsed, "pipelineId": self.pipeline_id})
# # #             ai_event("rag_query", properties={"correlation_id": run_cid, "query_len": len(query), "top_k": top_k, "pipelineId": self.pipeline_id})
# # #             return result
# # #         except Exception as e:
# # #             elapsed = time.time() - start_ts
# # #             ai_exception(e, properties={"correlation_id": run_cid, "elapsed_s": elapsed, "pipelineId": self.pipeline_id})
...
# # #     async def setup_vector_index_from_url(self, url: str) -> None:
# # #         ai_trace("setup_vector_index_from_url_start", properties={"url": url, "pipelineId": self.pipeline_id})
# # #         print("🌐 Fetching and indexing filing...")
# # #         self.source_text = await self._fetch_and_parse_10q(url)
# # #         await self._build_vector_index(self.source_text)
# # #         print("✅ Vector index ready.\n")
# # #         ai_trace("setup_vector_index_from_url_done", properties={"url": url, "pipelineId": self.pipeline_id})

# # #     async def setup_vector_index_from_text(self, text: str) -> None:
# # #         ai_trace("setup_vector_index_from_text_start", properties={"text_len": len(text), "pipelineId": self.pipeline_id})
# # #         print("🧾 Indexing uploaded text...")
# # #         self.source_text = text
# # #         await self._build_vector_index(self.source_text)
# # #         print("✅ Vector index ready from text.\n")
# # #         ai_trace("setup_vector_index_from_text_done", properties={"text_len": len(text), "pipelineId": self.pipeline_id})

# # #     async def run_single_agent(self, cfg: Dict[str, Any], reflection_agent, credential, project_endpoint: str):
# # #         run_cid = str(uuid.uuid4())
# # #         start_time = time.time()
# # #         ai_trace("run_single_agent_start", properties={"correlation_id": run_cid, "agent_name": cfg.get("name"), "agent_id": cfg.get("id"), "pipelineId": self.pipeline_id})
# # #         try:
# # #             client = await create_client(credential, project_endpoint, cfg["id"])
# # #             agent = ChatAgent(chat_client=client, instructions=cfg["base_prompt"])
...
# # #             return cfg["name"], final_output.strip()
# # #         except Exception as e:
# # #             ai_exception(e, properties={"correlation_id": run_cid, "agent_name": cfg.get("name"), "pipelineId": self.pipeline_id})
# # #             print(f"❌ {cfg['name']} failed: {e}")
# # #             return cfg["name"], f"Error: {e}"

# # #     async def run_agent_pipeline(self, company_url: Optional[str] = None):
# # #         pipeline_cid = str(uuid.uuid4())
# # #         ai_trace("run_agent_pipeline_start", properties={"correlation_id": pipeline_cid, "company_url": company_url, "pipelineId": self.pipeline_id})
# # #         if company_url:
# # #             await self.setup_vector_index_from_url(company_url)
...
# # #             ai_event("pipeline_completed", properties={"correlation_id": pipeline_cid, "result_count": len(results), "pipelineId": self.pipeline_id})
# # #             try:
# # #                 tc.flush()
# # #             except Exception:
# # #                 logger.debug("Failed to flush telemetry", exc_info=True)
...
# # # async def run_full_pipeline_from_url(url: str, similarity_top_k: int = 5) -> Dict[str, Any]:
# # #     symbol = extract_symbol_from_url_or_filename(url)
# # #     pipeline_id = make_pipeline_id(symbol)
# # #     pipeline = FinancialRAGMAgenticSystem(
# # #         azure_endpoint="https://aif-creditai-qa-001.cognitiveservices.azure.com/",
# # #         api_key="Au4vNbOXpEX2DPvxp7eH7CX5utrj5PE043biXOss78GyizT8fNMHJQQJ99BJACHYHv6XJ3w3AAAAACOGvY4U",
# # #         embedding_deployment="text-embedding-3-large",
...
# # # async def run_full_pipeline_from_text(text: str, filename: Optional[str] = None, similarity_top_k: int = 5) -> Dict[str, Any]:
# # #     symbol = extract_symbol_from_url_or_filename(filename or "")
# # #     pipeline_id = make_pipeline_id(symbol)
# # #     pipeline = FinancialRAGMAgenticSystem(
# # #         azure_endpoint="https://aif-creditai-qa-001.cognitiveservices.azure.com/",
# # #         api_key="Au4vNbOXpEX2DPvxp7eH7CX5utrj5PE043biXOss78GyizT8fNMHJQQJ99BJACHYHv6XJ3w3AAAAACOGvY4U",
...
#     """
#     Create a stable pipeline ID using the input name (ticker/file)
#     + current date and time, no randomness.
#     """
#     if not name:
#         name = "pipeline"
...
#     # Timestamp (year-month-day_hour-minute-second)
#     timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")

#     return f"pipeline_{cleaned}_{timestamp}"
...
#         return match.group(1).upper()
#     parsed = urlparse(path)
#     filename = parsed.path.split("/")[-1] if parsed.path else path
#     filename_no_ext = re.sub(r"\.[A-Za-z0-9]+$", "", filename)
#     symbol = extract_symbol_from_text(filename_no_ext)
#     return symbol.upper() if symbol else ""
...
# # FinancialRAGMAgenticSystem class with stable pipeline_id usage
# class FinancialRAGMAgenticSystem:
#     def __init__(
#         self,
#         azure_endpoint: str,
...
#     async def _fetch_and_parse_10q(self, url: str) -> str:
#         start_ts = time.time()
#         ai_trace("fetch_and_parse_10q_start", properties={"correlation_id": self.pipeline_id, "url": url, "pipelineId": self.pipeline_id})
#         def blocking_fetch() -> str:
#             headers = {
#                 "User-Agent": "Sachin Bhusnurmath (sachin@example.com)",
#                 "Accept-Encoding": "gzip, deflate",
...
#             text = await asyncio.get_event_loop().run_in_executor(self.executor, blocking_fetch)
#             elapsed = time.time() - start_ts
#             ai_trace("fetch_and_parse_10q_success", properties={"correlation_id": self.pipeline_id, "elapsed_s": elapsed, "url": url, "pipelineId": self.pipeline_id})
#             return text
#         except Exception as e:
#             elapsed = time.time() - start_ts
#             ai_exception(e, properties={"correlation_id": self.pipeline_id, "elapsed_s": elapsed, "url": url, "pipelineId": self.pipeline_id})
#             raise

#     async def _build_vector_index(self, text: str):
#         start_ts = time.time()
#         ai_trace("build_vector_index_start", properties={"correlation_id": self.pipeline_id, "text_len": len(text), "pipelineId": self.pipeline_id})
#         def blocking_build():
#             doc = Document(text=text)
#             node_parser = SimpleNodeParser.from_defaults(chunk_size=1024, chunk_overlap=256)
#             nodes = node_parser.get_nodes_from_documents([doc])
...
#             self.vector_index = index
#             elapsed = time.time() - start_ts
#             ai_trace("build_vector_index_success", properties={"correlation_id": self.pipeline_id, "elapsed_s": elapsed, "node_count": len(index._store._nodes) if hasattr(index, "_store") else None, "pipelineId": self.pipeline_id})
#             return index
#         except Exception as e:
#             elapsed = time.time() - start_ts
#             ai_exception(e, properties={"correlation_id": self.pipeline_id, "elapsed_s": elapsed, "pipelineId": self.pipeline_id})
#             raise

#     async def query_vector_index(self, query: str, top_k: int = 3) -> str:
#         start_ts = time.time()
#         ai_trace("query_vector_index_start", properties={"correlation_id": self.pipeline_id, "query": query, "top_k": top_k, "pipelineId": self.pipeline_id})
#         def blocking_query():
#             query_engine = self.vector_index.as_query_engine(
#                 llm=self.llm, similarity_top_k=top_k, response_mode="compact"
#             )
...
#             result = await asyncio.get_event_loop().run_in_executor(self.executor, blocking_query)
#             elapsed = time.time() - start_ts
#             ai_trace("query_vector_index_success", properties={"correlation_id": self.pipeline_id, "elapsed_s": elapsed, "pipelineId": self.pipeline_id})
#             ai_event("rag_query", properties={"correlation_id": self.pipeline_id, "query_len": len(query), "top_k": top_k, "pipelineId": self.pipeline_id})
#             return result
#         except Exception as e:
#             elapsed = time.time() - start_ts
#             ai_exception(e, properties={"correlation_id": self.pipeline_id, "elapsed_s": elapsed, "pipelineId": self.pipeline_id})
...
#     async def run_single_agent(self, cfg: Dict[str, Any], reflection_agent, credential, project_endpoint: str):
#         ai_trace("run_single_agent_start", properties={"agent_name": cfg.get("name"), "agent_id": cfg.get("id"), "pipelineId": self.pipeline_id})
#         try:
#             client = await create_client(credential, project_endpoint, cfg["id"])
#             agent = ChatAgent(chat_client=client, instructions=cfg["base_prompt"])
...
#             ai_event("agent_input", properties={"agent_name": cfg.get("name"), "agent_id": cfg.get("id"), "prompt_input": (prompt_input[:4000] if prompt_input else ""), "pipelineId": self.pipeline_id})

#             _, _, final_output = await run_generator_with_reflection(
#                 agent, reflection_agent, prompt_input, cfg["revise_prompt"]
#             )

#             ai_event("agent_output", properties={"agent_name": cfg.get("name"), "agent_id": cfg.get("id"), "output_len": len(final_output) if final_output else 0, "pipelineId": self.pipeline_id})

#             ai_trace(f"agent_output_full:{cfg.get('name')}", properties={"agent_name": cfg.get("name"), "output": (final_output[:8000] if final_output else ""), "pipelineId": self.pipeline_id})

#             print(f"✅ Finished {cfg['name']}")
#             return cfg["name"], final_output.strip()
#         except Exception as e:
#             ai_exception(e, properties={"agent_name": cfg.get("name"), "pipelineId": self.pipeline_id})
#             print(f"❌ {cfg['name']} failed: {e}")
#             return cfg["name"], f"Error: {e}"

#     async def run_agent_pipeline(self, company_url: Optional[str] = None):
#         ai_trace("run_agent_pipeline_start", properties={"company_url": company_url, "pipelineId": self.pipeline_id})
#         if company_url:
#             await self.setup_vector_index_from_url(company_url)

#         async with AzureCliCredential() as credential:
#             project_endpoint = "https://aif-creditai-qa-001.services.ai.azure.com/api/projects/aif-proj-creditai-qa-001"
...
#             ai_event("pipeline_completed", properties={"result_count": len(results), "pipelineId": self.pipeline_id})
#             try:
#                 tc.flush()
#             except Exception:
#                 logger.debug("Failed to flush telemetry", exc_info=True)
...
# async def create_client(credential, project_endpoint: str, agent_id: str):
#     ai_trace("create_client", properties={"project_endpoint": project_endpoint, "agent_id": agent_id})
#     return AzureAIAgentClient(
#         async_credential=credential,
#         project_endpoint=project_endpoint,
#         agent_id=agent_id,
...
#     revision_prompt: str,
# ):
#     ai_trace("run_generator_with_reflection_start", properties={})

#     thread = generator_agent.get_new_thread()
#     ai_event("generator_input", properties={"input_len": len(input_text) if input_text else 0})

#     gen_response = await generator_agent.run(input_text, thread=thread)
#     ai_event("generator_response", properties={"response_len": len(gen_response.text) if getattr(gen_response, "text", None) else 0})
#     ai_trace("generator_response_text", properties={"text_snippet": (gen_response.text[:4000] if getattr(gen_response, "text", None) else "")})

#     reflect_input = f"{REFLECTION_PROMPT}\n\nGenerator Output:\n{gen_response.text}"
#     ai_event("reflection_input", properties={"input_len": len(reflect_input)})

#     reflect_response = await reflection_agent.run(reflect_input, thread=thread)
#     ai_event("reflection_response", properties={"response_len": len(reflect_response.text) if getattr(reflect_response, "text", None) else 0})
#     ai_trace("reflection_response_text", properties={"text_snippet": (reflect_response.text[:2000] if getattr(reflect_response, "text", None) else "")})

#     revise_input = f"{revision_prompt}\nReflection feedback:\n{reflect_response.text}"
#     ai_event("revision_input", properties={"input_len": len(revise_input)})

#     revised_response = await generator_agent.run(revise_input, thread=thread)
#     ai_event("revision_response", properties={"response_len": len(revised_response.text) if getattr(revised_response, "text", None) else 0})
#     ai_trace("revision_response_text", properties={"text_snippet": (revised_response.text[:4000] if getattr(revised_response, "text", None) else "")})

#     ai_trace("run_generator_with_reflection_done", properties={})
#     return gen_response.text, reflect_response.text, revised_response.text
...
# def extract_text_from_file(file_obj, filename: Optional[str] = None, file_type: Optional[str] = None) -> str:
#     ai_trace("extract_text_from_file_start", properties={"filename": filename})
#     try:
#         ftype = file_type or getattr(file_obj, "type", None) or _infer_mime_from_name(filename)
#         buffer = None
#         if isinstance(file_obj, io.BytesIO):
...
#             buffer.seek(0)
#         else:
#             ai_trace("extract_text_from_file_unsupported_type", properties={"file_type": ftype or "unknown"})
#             return ""
...
#             except Exception as pe:
#                 logger.error("Unable to read PDF: %s", pe)
#                 ai_exception(pe, properties={"filename": filename})
#                 return ""
#             for page in pdf_reader.pages:
#                 page_text = page.extract_text() or ""
#                 text += page_text + "\n"
...
#             ai_trace("extract_text_from_file_unsupported_type", properties={"file_type": ftype or "unknown"})
#             return ""
#         ai_trace("extract_text_from_file_success", properties={"text_len": len(text)})
#         return text.strip()
#     except Exception as e:
#         logger.exception("Error reading file")
#         ai_exception(e)
...
# def build_latest_10q_url_from_mapping(ticker: str, mapping_json_path: str) -> Optional[str]:
#     ai_trace("build_latest_10q_url_from_mapping_start", properties={"ticker": ticker})
#     try:
#         with open(mapping_json_path, "r") as f:
#             mapping = json.load(f)
#         df = pd.DataFrame(mapping["data"], columns=mapping["fields"])
...
#         doc_name = q.primaryDocument.values[0]
#         html_url = f"https://www.sec.gov/Archives/edgar/data/{cik}/{acc_num}/{doc_name}"
#         ai_trace("build_latest_10q_url_from_mapping_success", properties={"html_url": html_url})
#         return html_url
#     except Exception:
#         logger.exception("Failed building 10-Q URL from mapping")
#         ai_exception(Exception("Failed building 10-Q URL from mapping"))
#         return None
import os
import io
import re
...
class FinancialRAGMAgenticSystem:
    ...
    async def run_single_agent(self, cfg: Dict[str, Any], reflection_agent, credential, project_endpoint: str):
        ...
        try:
            ...
            ai_event("agent_input", properties={"agent_name": cfg.get("name"), "agent_id": cfg.get("id"), "prompt_input": (prompt_input[:8000] if prompt_input else ""), "pipelineId": self.pipeline_id})
            ...
            ai_trace(f"agent_output_full:{cfg.get('name')}", properties={"agent_name": cfg.get("name"), "output": (final_output[:10000] if final_output else ""), "pipelineId": self.pipeline_id})
...
async def run_generator_with_reflection(
    ...
    gen_response = await generator_agent.run(input_text, thread=thread)
    ai_event("generator_response", properties={"response_len": len(gen_response.text) if getattr(gen_response, "text", None) else 0})
    ai_trace("generator_response_text", properties={"text_snippet": (gen_response.text[:8000] if getattr(gen_response, "text", None) else "")})
    ...
    reflect_response = await reflection_agent.run(reflect_input, thread=thread)
    ai_event("reflection_response", properties={"response_len": len(reflect_response.text) if getattr(reflect_response, "text", None) else 0})
    ai_trace("reflection_response_text", properties={"text_snippet": (reflect_response.text[:8000] if getattr(reflect_response, "text", None) else "")})
    ...
    revised_response = await generator_agent.run(revise_input, thread=thread)
    ai_event("revision_response", properties={"response_len": len(revised_response.text) if getattr(revised_response, "text", None) else 0})
    ai_trace("revision_response_text", properties={"text_snippet": (revised_response.text[:8000] if getattr(revised_response, "text", None) else "")})
